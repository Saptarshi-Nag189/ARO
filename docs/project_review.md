...
> all of **P0** (2.1 max-iterations enforcement, 2.2 f-string crash, 2.3 single-worker gunicorn, 2.6 session eviction, 2.7 `.dockerignore` + non-root user, 2.8 log-handler leak), all of **P1** (2.4 novelty recompute, 2.5 contradiction/gap resolution wired end-to-end via new `SynthesisOutput.resolved_contradictions`/`resolved_gap_ids` — including dedupe of re-reported contradiction pairs, 2.10 conclusion via `ModelGateway.call_text()`, 2.11 real prior-art scan on Semantic Scholar + OpenAlex with lexical-overlap similarity, 2.12 no fabricated linkage, 2.14 source dedup by URL + `corroborating_source_ids` through claim merges), plus from **P2/P3**: 2.9 cross-session memory read path (prior knowledge injected into planner/research prompts), 2.13 UI auth (`ui/src/api.js`; SSE via `api_key` query param), 2.15 API mode validation, a **pytest suite (`tests/`) + GitHub Actions CI**, dependency cleanup (dropped `google-adk`/`sqlalchemy`, `duckduckgo-search`→`ddgs`, `pytest>=8`), and resolve-then-verify SSRF protection (2.20).
> **All findings are now closed.** The LangGraph v3 rewrite (PR #6) superseded parts of this review — it implemented interactive mode as a real `interrupt()` (2.15), replaced the monkey-patched SSE with an emitter in `graph/services.py` (3.1), and removed the v2 orchestrator/gateway entirely. The final cleanup pass then deleted the remaining dead modules (`event_bus.py`, `metrics_engine.py`, `search_tool.py`, unused async search wrappers and caches — 2.16), fixed skeptic credibility-challenge targeting for both claim and source IDs (3.4), swept `datetime.utcnow()` to timezone-aware calls, moved both SQLite files under `data/` with a Docker volume, added per-IP rate limiting on `/api/run`, and made `TTLCache` thread-safe. Line numbers refer to the commit the review was written against; the architecture described in §1 is the pre-LangGraph v2 — see `docs/langgraph_migration.md` for what replaced it.
